...
**Before AV proposes anything: research current agentic framework options.** Your training data has a cutoff, and the agentic framework landscape (LangGraph, CrewAI, AutoGen-successors, vendor-native agent SDKs, raw tool-calling against a model API directly, and whatever has emerged since) moves fast enough that a framework choice from memory risks being stale the day it's written. Web-search the current state of the relevant options for this project's needs — orchestration style, state management, ecosystem maturity, whether the project even needs a framework versus a simpler hand-rolled loop — before AV makes a recommendation. This is not optional research to skip if you feel confident; treat your own framework familiarity as a starting hypothesis to verify, not a conclusion.

This same research discipline applies to anything else in Phase 2 with a fast-moving ecosystem: if AV is about to recommend a specific vector DB, LLM provider SDK version, or adapter pattern, a quick check that the recommendation isn't built on a deprecated API or superseded library is cheap insurance against handing Builder a spec built on stale assumptions.

AV proposes, now informed by that research:
- Orchestration shape (single agent, multi-agent with explicit roles, graph-based with state, etc.) — and which framework or library this uses, if any, with a one-sentence justification citing what the research found
...
- Is the chosen framework (if any) actually justified, or is this defaulting to whatever's familiar instead of what the research surfaced as current best fit?
...
### Framework Choice
- Framework/library used (if any): [name, version/release as of spec date]
- Research basis: [one sentence — what the Phase 2 research found that justified this, or "hand-rolled, no framework needed because..."]
- Date checked: [date this was researched — flags to future readers if this needs re-verification before relying on it]
